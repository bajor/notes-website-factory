...
test-runtime:
	...
	normal_dom="$$("$(CHROMIUM)" $(BROWSER_FLAGS) --dump-dom "$(RUNTIME_TEST_URL)")" && printf '%s' "$$normal_dom" | grep -Fq 'class="scene-link scene-game-link"' && printf '%s' "$$normal_dom" | grep -Fq 'aria-label="Play example game in Algo Arcade"' && printf '%s' "$$normal_dom" | grep -Fq 'aria-label="Play %FF in Algo Arcade"' && printf '%s' "$$normal_dom" | grep -Fq 'target="_blank" rel="noopener noreferrer"' && printf '%s' "$$normal_dom" | grep -Fq 'class="scene-game-icon"' && printf '%s' "$$normal_dom" | grep -Fq 'aria-label="Play YouTube video"' && printf '%s' "$$normal_dom" | grep -Fq 'stroke-miterlimit="4"' && printf '%s' "$$normal_dom" | grep -Fq 'stroke-dasharray="6 3"' && printf '%s' "$$normal_dom" | grep -Fq 'stroke-dashoffset="1"' && printf '%s' "$$normal_dom" | grep -Fq 'transform="matrix(0, 20, 30, 0, 50, 90)"'
