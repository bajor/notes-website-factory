...
Maintain an educational Haskell software factory that converts the repository's one-page Apple Freeform PDF into a static JavaScript DOM/Canvas site.
...
- `dist/` must not contain a PDF, full-page PDF raster, OCR output, SVG rendering, or browser PDF parser.
- Embedded raster content stays raster. Only genuine supported PDF text operators become DOM text.
...
- `generator/src/Factory/Pdf.hs`: `pdf-toolbox` boundary, stream decoding, resources, assets, and annotations.
...
- visual error and ink metrics meet the constants in `Factory.Evaluation`;
