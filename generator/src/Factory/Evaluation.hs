...
-- screenshot of our generated DOM/Canvas scene and keep the difference image
...
runVisualEvaluation pdfPath siteDirectory reportDirectory = do
  ...
  let referenceStem = reportDirectory </> "reference"
      referencePath = referenceStem <> ".png"
      generatedPath = reportDirectory </> "generated.png"
      differencePath = reportDirectory </> "difference.png"
  referenceExit <- runTool "pdftoppm" ["-singlefile", "-png", "-r", show referenceDpi, pdfPath, referenceStem]
    ...
    Right () -> do
      ...
      case reference of
        ...
        Right referenceImage -> do
          browserExit <- runBrowser absoluteSite generatedPath (imageWidth referenceImage) (imageHeight referenceImage)
          case browserExit of
            Left message -> pure (Left (EvaluationError message))
            Right () -> compareImages referenceImage generatedPath differencePath reportDirectory

runBrowser :: FilePath -> FilePath -> Int -> Int -> IO (Either Text ())
runBrowser siteDirectory screenshotPath width height = do
  ...
  where
    pageUrl = "file://" <> siteDirectory </> "index.html" <> "?evaluation"
    browserArguments =
      [ "--headless"
      , "--no-sandbox"
      , "--disable-gpu"
...
compareImages :: Image PixelRGB8 -> FilePath -> FilePath -> FilePath -> IO (Either BuildError EvaluationResult)
compareImages reference generatedPath differencePath reportDirectory = do
  ...
  case generated of
    ...
    Right generatedImage
      ...
      | otherwise -> do
          ...
          writeReports reportDirectory result
          pure (Right result)
...
writeReports :: FilePath -> EvaluationResult -> IO ()
writeReports directory result = do
  ...
  where
    ...
    html =
      Text.unlines
        ...
        , "<h2>Reference</h2><img src=\"reference.png\" width=\"48%\">"
        , "<h2>Generated</h2><img src=\"generated.png\" width=\"48%\">"
        , "<h2>Amplified difference</h2><img src=\"difference.png\" width=\"48%\">"
        , "</body></html>"
        ]
